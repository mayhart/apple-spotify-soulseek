...
namespace Spotify.Slsk.Integration.Services.SoulSeek
{
    public class SoulseekService
    {
            ...
            SoulseekOptions options, string? folderName = null)
        {
            ...
            Soulseek.File? file = SelectFile(response.Files, options, trackToDownload.EffectiveDurationMs / 1000);
            if (file == null)
            {
            ...
            string desiredFileName = trackToDownload.DesiredFileName
                ?? (trackToDownload.Track != null
                    ? GetDesiredFileName(trackToDownload.Track.Track!)
                    : file.Filename.RemoveSpecialCharacters());
        ...
        private static bool IsValidMp3(Soulseek.File file)
        {
            ...
            return file != null
                ...
                && file.Size * 8 / file.Length / 1000 >= minimalBitRate
        ...
        private static async Task<SoulseekResult> DownloadFileAsync(SoulseekClient client, string username, string file, string desiredFileName)
        {
            ...
            try
            {
                ...
                Transfer? transfer = await client.DownloadAsync(username, file, completeFilePath, startOffset: 0, token: Index++, options: new TransferOptions(
                    stateChanged: (e) =>
                    {
