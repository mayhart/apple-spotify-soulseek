...
{
    [Command(Name = "download-playlist", Description = "searches all spotify playlist tracks on soulseek and checks for available high quality mp3s")]
    class DownloadPlaylistCommand : SpotSeekCommandBase
    {
        ...
        public DownloadPlaylistCommand(ILogger<DownloadPlaylistCommand> logger, IConsole console)
        {
            _logger = logger;
            _console = console;
        }
        ...
        protected override async Task<int> OnExecute(CommandLineApplication app)
        {
            ...
            try
            {
                ...
                await _downloadService.DownloadAllPlaylistTracksFromUserAsync(SpotifyUserId,
                    SpotifyPlaylistId,
                    SpotifyPlaylistName,
                    SSUsername,
                    SSPassword,
